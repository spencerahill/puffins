...
P_INTERP_VALS = np.arange(0, 1000.5, 1.0)
...
def interp_p(arr, new_p_vals=P_INTERP_VALS, method="linear", p_str=LEV_STR):
    """Interpolate an array to new pressure values."""
    return arr.interp(**{p_str: new_p_vals}, method=method)


def interp_arrs_in_p(
    arrs, new_p_vals=P_INTERP_VALS, method="cubic", lat_str=LAT_STR, p_str=LEV_STR
):
...
def _maybe_interp_in_p(arrs, new_p_vals=P_INTERP_VALS, do_interp=True, p_str=LEV_STR):
...
def drop_nans_and_interp(
    arrs, new_p_vals=P_INTERP_VALS, do_interp=True, nan_drop_dim=LAT_STR, p_str=LEV_STR
):
...
def interp_ds_p_to_p(
    ds, plevs, method="cubic", pfull_str=PFULL_STR, lev_str=LEV_STR, lat_str=LAT_STR
):

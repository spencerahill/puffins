...
# Tropopause height.
def trop_height_fixed_tt(
    temp_sfc, temp_tropo=TEMP_TROPO, gamma=GAMMA_MOIST, grav=GRAV_EARTH, c_p=C_P
):
...
def pot_temp_fixed_lapse_rate(
    temp_sfc, height, gamma=GAMMA_MOIST, grav=GRAV_EARTH, c_p=C_P
):
    """Potential temperature at a given height if lapse rate is constant."""
    invgam = 1.0 / gamma
    return temp_sfc**invgam * (temp_sfc - gamma * grav / c_p * height) ** (1 - invgam)


def pot_temp_tropopause_fixed_tt(temp_sfc, temp_tropo=TEMP_TROPO, gamma=GAMMA_MOIST):
    """Potential temperature at the tropopause, given a fixed lapse rate."""
    invgam = 1.0 / gamma
    return temp_sfc**invgam * temp_tropo ** (1 - invgam)


def pot_temp_avg_fixed_lapse_rate(
    temp_sfc, height, gamma=GAMMA_MOIST, grav=GRAV_EARTH, c_p=C_P
):
    ...
    return (
        (temp_sfc**invgam)
        / ((2 - invgam) * gamma * gamma_dry * height)
        * (
            temp_sfc ** (2 - invgam)
...
def pot_temp_avg_fixed_lapse_rate_temp_tropo(
    temp_sfc, temp_tropo=TEMP_TROPO, gamma=GAMMA_MOIST
):
...
def temp_sfc_fixed_lapse_rate_height_tropo(
    pot_temp_avg,
...
def temp_sfc_fixed_lapse_rate_temp_tropo(
    pot_temp_avg,
...
def dpot_temp_avg_dtemp_sfc_fixed_lapse_rate(
    temp_sfc, gamma=GAMMA_MOIST, temp_tropo=TEMP_TROPO
):
    ...
    term2 = (
        ((1 - gamma) * ts - tt) * (ts ** (2 - invgam) - tt ** (2 - invgam)) / (ts - tt)
    )
...
def grad_wind_bouss_fixed_temp_tropo(
    ...
    sqrt_term = 1 - leading_factor * (
        temp_drop * dtheta_dlat - pot_temp_drop * dtemp_sfc_dlat
    )
    return rot_rate * radius * coslat * (np.sqrt(sqrt_term) - 1)


def temp_sfc_amc_fixed_tt_bouss(
    lat,
    lat_ascent,
    temp_sfc_ascent,
    theta_ref=THETA_REF,
    ...
    return temp_tropo + np.sqrt((temp_sfc_ascent - temp_tropo) ** 2 - chi * lat_factor)


def pot_temp_avg_amc_fixed_tt_bouss(
    lat,
    lat_ascent,
    temp_sfc_ascent,
    theta_ref=THETA_REF,
...
def _theta_b_amc_root(
    ...
    rhs = theta_ascent - (((rot_rate**2 * radius**2) / (2.0 * c_p)) * lat_term)
    return guess - temp_tropo * np.log(guess / theta_ascent) - rhs


def _theta_b_amc_root_compute_temp_sfc(
    guess,
    lat,
    theta_ascent,
    lat_ascent,
    ...
    cos2lat = cosdeg(lat) ** 2
    rhs = -(
        ((rot_rate**2 * radius**2) / (2.0 * c_p))
        * ((cosdeg(lat_ascent) ** 2 - cos2lat) ** 2)
        / cos2lat
    )
    temp_sfc = temp_from_equiv_pot_temp(guess, rel_hum=1.0, pressure=700e2)
    lhs = temp_sfc * (1 - theta_ascent / guess) + temp_tropo * np.log(
        theta_ascent / guess
    )
    return lhs - rhs


def pot_temp_amc_cqe_fixed_tt(
    lats,
    bound_guess_range,
    pot_temp_ascent,
    lat_ascent,
    ...
    return brentq_solver_sweep_param(
        func, lats, init_guess, bound_guess_range, funcargs=funcargs
    )

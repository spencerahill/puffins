...
def pot_temp_rce_ph92(
    lat, theta_max_factor, lat_max=25, nonzero_width=15, temp_ref=THETA_REF
):
    """Vertical average of Eq. 9 of Plumb and Hou 1992."""
    nonzero_region = (
        temp_ref
        + theta_max_factor * cosdeg(90.0 * (lat - lat_max) / nonzero_width) ** 2
    )
    flat_region = xr.ones_like(lat) * temp_ref
    return xr.where(
        (lat < lat_max + nonzero_width) & (lat > lat_max - nonzero_width),
        nonzero_region,
        flat_region,
    )

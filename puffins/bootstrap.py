...
def bootstrap_samples(arr, dim, num_bootstraps=1000, rand_states=None):
    ...
    if rand_states is None:
        rand_states = np.random.default_rng().integers(0, 4e9, size=num_bootstraps)
    return [
...
def corr_sig_nonzero_from_full_and_boot(
    corr_full, corrs_boot, alpha=0.05, dim_boot="nboot"
):
...
def corr_sig_nonzero_bootstrap(
    arr1, arr2, dim, num_bootstraps=1000, alpha=0.05, dim_boot="nboot"
):
...
def boot_risk_ratio(
    arr, num_numer, num_denom, dim, cdf_points, num_bootstraps=1000, side="left"
):

...
def kuo_el_eddy_mom_term(
    ...
    return (
        2 * rot_rate * sindeg(lat) * d2uv_eddy_cos2lat_dp_dlat / (radius * coslat**2)
    ).transpose(*u_merid_flux_eddy.dims)


def kuo_el_eddy_temp_term(
    temp, temp_eddy_merid_flux, pressure, r_d=R_D, radius=RAD_EARTH, lat_str=LAT_STR
):
    ...
    d2temp_eddy_flux_dlat2_term = lat_deriv(
        dtemp_eddy_merid_flux_dlat / coslat, lat_str
    )
    return -r_d / (pressure * radius**2) * d2temp_eddy_flux_dlat2_term


def kuo_el_fric_term(
    zonal_friction,
    pressure,
    rot_rate=ROT_RATE_EARTH,
    vert_str=SIGMA_STR,
    ...
    return -d_zonal_fric_dp * coriolis_param(zonal_friction[lat_str], rot_rate=rot_rate)


def kuo_el_diab_term(
    diab_heat, pressure, r_d=R_D, c_p=C_P, radius=RAD_EARTH, p0=P0, lat_str=LAT_STR
):
...
def _kuo_el_matrix(
    ...
    chi = coord_arr_1d(0, num_points - 1, 1, "chi", dtype=int)
    ...
    inv_coshalf_north = 1.0 / coslathalf_north
    inv_coshalf_south = 1.0 / coslathalf_south
    ...
    for n in range(num_j, num_points - num_j):
        ...
        # For gridoint (k, j).
        row[n] = (
            nu_vals[k] * (inv_coshalf_south[j] + inv_coshalf_north[j]) - 2 * al_vals[j]
        )
        # For gridpoint (k-1, j).
...
def _kuo_el_apply_bc(arr, lat_str=LAT_STR, lev_str=LEV_STR, bc=0.0):
...
def kuo_el_solver(
    ...
    ke_sol_vals = sor_solver(
        ...
        _kuo_el_apply_bc(forcing, lat_str=lat_str, lev_str=lev_str).values.flatten(),
        init_guess.flatten(),
        omega=omega,
        conv_crit=conv_crit,
        verbose=verbose,

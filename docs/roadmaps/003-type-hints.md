...
- [x] `vert_coords.py` — vertical coordinate transformations; 62 tests, originally 3 xfail for the pre-existing `xr.concat` bug #26 in `pfull_simm_burr`. #26 was fixed in PR #60 (the top level is now `expand_dims`'d and labeled to match, so `xr.concat` works for both scalar- and dimension-coordinate `pfull` inputs, and `pfull_vals_simm_burr` gained a decreasing-pressure branch so it agrees with the wrapper); those three tests were un-xfailed (type hints completed 2026-03-29; #26 fixed 2026-07-19)
